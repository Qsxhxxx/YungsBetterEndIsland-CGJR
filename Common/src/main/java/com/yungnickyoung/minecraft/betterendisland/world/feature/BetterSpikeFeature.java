...
public class BetterSpikeFeature {
    ...
    public static List<SpikeFeature.EndSpike> getSpikesForLevel(WorldGenLevel level) {
        long seed = level.getSeed() ^ (level.getSeed() >>> 16);
        return SPIKE_CACHE.getUnchecked(seed & 65535L);
    }
    ...
    public static void placeSpike(ServerLevelAccessor level, RandomSource randomSource, SpikeConfiguration config, SpikeFeature.EndSpike spike, boolean isInitialSpawn) {
        ...
        // Place top part - use adaptive height based on actual terrain at spike location (+2 offset to prevent air blocks from destroying ground)
        int cx = spike.getCenterX();
        int cz = spike.getCenterZ();
        int topY = getSpikeTopY(level, cx, cz) + 2;
        BlockPos centerPos = new BlockPos(cx, topY, cz);
    ...
    private static boolean placeTemplate(ServerLevelAccessor level, RandomSource randomSource, BlockPos centerPos, Rotation rotation, ResourceLocation id, int numberTimesDragonKilled) {
        ...
        BetterEndIslandCommon.betterendisland$placingOwn.set(true);
        try {
            template.placeInWorld(level, cornerPos, centerPos, structurePlaceSettings, randomSource, 2);
        } finally {
            BetterEndIslandCommon.betterendisland$placingOwn.set(false);
        }
        return true;
    }

    private static int getSpikeTopY(ServerLevelAccessor level, int x, int z) {
        BlockPos.MutableBlockPos mutable = new BlockPos.MutableBlockPos();
        int maxY = level instanceof WorldGenLevel wgl ? Math.min(wgl.getHeight(), level.getMaxBuildHeight()) : level.getMaxBuildHeight();
        for (int y = Math.min(maxY, 128); y > level.getMinBuildHeight(); y--) {
            mutable.set(x, y, z);
            if (level.getBlockState(mutable).is(Blocks.END_STONE)) {
                return y;
            }
        }
        return BetterEndIslandCommon.betterEnd ? 70 : 60;
    }
}
